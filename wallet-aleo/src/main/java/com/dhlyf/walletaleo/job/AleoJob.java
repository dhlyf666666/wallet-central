...
public class AleoJob {
    ...
    public void scanBlock() {
        ...
        RedissonUtil.withLock("AleoJob.scanBlock", () -> {
    ...
    public void pullWithdraw() {
        ...
        RedissonUtil.withLock("AleoJob.pullWithdraw", () -> {
            MDC.put("uuid", UUID.randomUUID().toString().replaceAll("-", ""));
    ...
    public void pushWithdraw() {
        ...
        RedissonUtil.withLock("AleoJob.pushWithdraw", () -> {
            MDC.put("uuid", UUID.randomUUID().toString().replaceAll("-", ""));
            log.info("开始执行通知任务处理");
    ...
    public void handleWithdraw() {
        ...
        RedissonUtil.withLock("AleoJob.handleWithdraw", () -> {
            log.info("开始执行处理提现任务");
    ...
    public void cancelWithdraw() {
        ...
        RedissonUtil.withLock("AleoJob.cancelWithdraw", () -> {
            MDC.put("uuid", UUID.randomUUID().toString().replaceAll("-", ""));
            log.info("开始执行撤销提现任务");
    ...
    public void notifyTask() {
        ...
        RedissonUtil.withLock("AleoJob.notifyTask", () -> {
            MDC.put("uuid", UUID.randomUUID().toString().replaceAll("-", ""));
            log.info("开始执行通知任务处理");
...
//    充值归集
    ...
    public void collectDeposit() {
        ...
        RedissonUtil.withLock("AleoJob.collectDeposit", () -> {
            MDC.put("uuid", UUID.randomUUID().toString().replaceAll("-", ""));
            log.info("开始执行充值归集任务");
